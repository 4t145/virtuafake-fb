...
import SuprechatDataTableVue from "./SuprechatDataTable.vue";
...
<template>
    <DataTable :total="total" v-model:page="page">
        <template #table>
          ...
          <tbody>
            <tr v-for="(danmaku, index) in danmakus" :key="index">
              <td>
                {{
                  danmaku.data.message.tag === "Plain"
                    ? danmaku.data.message.data.message
                    : danmaku.data.message.data.alt_message
                }}
              </td>
